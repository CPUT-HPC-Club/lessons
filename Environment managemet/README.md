...
Understanding the utility and convenience of scripting, two scripting methods will be used. Run the first [ansible script](#ansible-installation) across all nodes **then** run the bash script from your head node (depending on the file system).

## Ansible installation

1. Create an inventory file and create the Ansible script `setup_Lmod_Ansible.yml`. It may start as follows,
    ```yml
    ---
...
2. Write to file `setup_Lmod_All.yml` so it installs all necessary prerequisite packages on all nodes as well as write `/etc/profile.d/lmod.sh` source script for each node. See the [bash script](#bash-script-installation) for a list of prerequisites.<br>

    > Using the `export` command stores values in volatile memory: changes apply only to the current shell and are not preserved when a new shell is launched. To make changes permanent, add the lines to `~/.bashrc`, `~/.bash_profile`, or another `.profile` file, or write `/etc/profile.d/lmod.sh`.
...
3. Run `setup_Lmod_All.yml` using command below.
    ...
    This does not mean Lmod is set up yet; the bash script must still be executed.
...
0. This step should be done by the ansible script but this a list of prerequisites and lmod dependencies.
    ...
    ```
    If Lmod was installed in a directory accessible by all nodes, `lua-filesystem` must be installed and `/etc/profile.d/lmod.sh` must be written for each node.
...
3. If profile script is not setup, here is an example extract how `/etc/profile.d/lmod.sh` may be written to in a bash script. The `module use --append` adds that directory modules to the front of the `MODULEPATH` environment variable; essentially the same as `export MODULEPATH=$BUILD_DIR/lmod/lmod/modulefiles:$MODULEFILES`.
    ...
    EOF
    ```
    Note `$BUILD_DIR` can be any directory, but this guide assumes `$BUILD_DIR="/home/software"` where `software` is a directory that every user has access to. And `$VERSION` is the Lmod version you are installing. You can check the [TACC GitHub repository](https://github.com/TACC/Lmod/releases) for the version you will be cloning.
...
> [!NOTE]
>
> Some installed packages will automatically add environment modules to the Lmod system, while others will not and will require you to manually add definitions for them. For example, the `Intel oneAPI Toolkits` package that we will install from source may not add environment modules automatically.
...
Lua modulefiles must have a `.lua` extension. This is an example of a Python Lua modulefile; a similar [tcl modulefile example](#tcl-modulefile-example).
```lua
...
Below is a list of some of the Lua functions you can use in your `<filename>.lua`. <br>

<details>
<summary>Defining module type using families</summary>

* **Module type/family:** A user can only have one family “name” loaded at a time. For example, family("compiler") means that a user could only have one compiler loaded at a time.
    ...
    > [!TIP]
    > 
    > family() can be used for preventing the user from loading multiple compilers.
    >
...
Below is a list of some of the Tcl functions you can use in your `<filename>` or `<filename>.tcl`.
<br>
